...
def merge_scroll_captures(captures: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    explored_height = max(
        view_height,
        max_bottom,
        max((row["scroll_y"] + row["height"] for row in scroll_slices), default=view_height),
    )
    # Canvas grows with explored scroll slices — not the full unread document height.
    ...
    base["scroll_map"] = {
        ...
        "slice_count": len(scroll_slices),
        "explored_height": canvas_height,
        "persistent_skipped": max(
            0, sum(len(c.get("elements") or []) for c in ordered) - len(merged_elements)
        ),
    ...
    # Prefer the top slice screenshot as the capture-level fallback image.
    top_shot = next(
...
def annotate_scroll_map(capture: dict[str, Any]) -> dict[str, Any]:
    ...
    # Single slice: canvas matches the viewport so element % layout stays correct.
    result["scroll_map"] = {
        "stitched": False,
        "coords": "viewport",
...
def accumulate_scroll_capture(
    ...
    *,
    ...
    # Keep the richest capture for a given scroll position.
    prior = by_scroll.get(scroll_y)
    if prior is None or len(capture.get("elements") or []) >= len(prior.get("elements") or []):
